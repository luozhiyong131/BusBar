    ...
    ui(new Ui::LineWid)
{
    ...
    mCheckDlg = new CheckPasswordDlg(this,false);
    connect(mCheckDlg,SIGNAL(dialogLockClosed(bool)),this,SLOT(dialogLockClosed(bool)));
...
void LineWid::dialogLockClosed(bool ret)
{
    if(ret)
    {
        mLock = true;
        ui->lockBtn->setText(tr("锁定"));
        emit lockStatus(mLock);
    }
    else
...
void LineWid::on_lockBtn_clicked()
{
    if(mLock)
    {
        ui->lockBtn->setText(tr("解锁"));
        mLock = false;
        QMessageBox::information(this,"information","锁定成功！","确认");
        emit lockStatus(mLock);
    }
    else
    {
