    ...
    ui(new Ui::HomeWid)
{
    ...
    ui->upBtn->setHidden(true);
    ui->downBtn->setHidden(true);
}
...
void HomeWid::timeoutDone()
{
    if(isRun) { //节省CPU的考虑
        ...
        int lzy = 5;
        if(mBusData->boxNum == 4) {
            lzy = 4;
        }

        QString str = "border-image: url(:/new/prefix1/image/branchs_%1.png);";
        ui->widget->setStyleSheet(str.arg(lzy));
    }
}
