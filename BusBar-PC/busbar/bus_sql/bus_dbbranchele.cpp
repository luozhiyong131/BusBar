...
BUS_DbBranchEle::BUS_DbBranchEle()
{
    mId=0;
}

void BUS_DbBranchEle::createTable()
{
    QString cmd =
...
bool BUS_DbBranchEle::insertItem(DbBranchEleItem& item)
{
    bool ret = false;
    item.id = maxId()+1;
...
bool BUS_DbBranchEle::modifyItem(const DbBranchEleItem &item, const QString &cmd)
{
    bool ret = false;
    QSqlQuery query;
    query.prepare(cmd);
...
void BUS_DbBranchEle::selectItem(QSqlQuery &query,DbBranchEleItem &item)
{
...
{
    static DbBranchEle1* sington1  = new DbBranchEle1();
